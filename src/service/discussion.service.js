...
router.get("/hi",passport.authenticate("jwt", {session: false}), (req,res)=>{
    res.send("HI")
})

router.get("/messagebox", (req,res)=>{
    Discussion.find({partners:{"$in" : [req.user.id]}}).then(descu=>{
        res.send(descu);
    }).catch(err=>{
        res.send(err)
    });
...
router.post("/sendmessage",(req,res)=>{
    ...
    Discussion.findOne({partners:{"$all" : [req.user.id,messageBody.partnerId]}}).then(discu=>{
        if(discu){
            message={sender:req.user.id,text:messageBody.text}
            discu.message.push(message);
            if(discu.isPaid){
                discu.save().then(sendmsg=>{console.log(sendmsg)}).catch(err=>console.log(err));
            ...
            if(STATICS_MSGS.includes(messageBody.text)){
                newDiscussion.save().then(savedDisc=>{console.log(savedDisc)}).catch(err=>{console.log(err)})
...
router.post("/buydiscussion", (req,res)=>{
    User.findById(req.user.id).then(user=>{
        if(user.oviv_currency>=PRICES.discussion){
            ...
            Discussion.findOne({partners:{"$all" : [req.user.id,req.body.partnerId]}}).then(discu=>{
                if(discu){
                    discu.isPaid = true;
...
router.get("/discussion", (req,res)=>{
    Discussion.findOne({partners:{"$all" : [req.user.id,req.body.partnerId]}}).then(discussion=>{
